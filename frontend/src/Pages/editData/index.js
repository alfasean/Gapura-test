...
function EditData() {
  ...
  useEffect(() => {
    const fetchExpeditions = async () => {
      try {
        const response = await axios.get('http://localhost:4000/expeditions');
    ...
    const fetchPackageById = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/packages/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:4000/packages/${id}`, formData);
      Swal.fire({
        icon: 'success',
        title: 'Sukses',
